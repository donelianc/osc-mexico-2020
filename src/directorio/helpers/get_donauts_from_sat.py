...
def get_donauts_from_sat(year, dir_path, save_response=True, filters=None):
    ...
    if filters == None:
        ...
        with open("./params/directorio-sat.json", "r") as f: params = load(f)

        extension = params[str_year]['file_extension']

        BASE = params[str_year]['base_url']
        URL = BASE + str_year + extension
...
#        response = s.get(URL, headers=headers)
        ...
        df = read_excel(
            dir_path + file_path
            , skiprows = range(
                params[str_year]['skip_rows'][0]
                , params[str_year]['skip_rows'][1]
                )
            , usecols = params[str_year]['usecols']
            )
